...
print("="*60)
print("CLASIFICACIÓN BREAST CANCER CON VOTING CLASSIFIER")
print("="*60)
...
# 3. DEFINIR CLASIFICADORES BASE Y PARÁMETROS PARA OPTIMIZACIÓN
print("\n3. CONFIGURANDO VOTING CLASSIFIER Y OPTIMIZACIÓN DE HIPERPARÁMETROS...")
...
# Crear el Voting Classifier con diferentes estrategias
...
# Crear el modelo base con todos los clasificadores
...
# Búsqueda en grid con validación cruzada (versión simplificada para eficiencia)
...
grid_search = GridSearchCV(
    base_voting, param_grid_simplified, 
    cv=cv_strategy, 
    scoring='accuracy', 
    n_jobs=-1, 
    verbose=1
...
for metric in scoring_metrics:
    scores = cross_val_score(best_voting, X_train_scaled, y_train, cv=cv_strategy, scoring=metric)
...
y_pred_proba = best_voting.predict_proba(X_test_scaled)[:, 1]
...
# Evaluación individual de clasificadores
print("\n" + "="*50)
print("RENDIMIENTO DE CLASIFICADORES INDIVIDUALES:")
print("="*50)

individual_scores = {}
for name, clf in best_voting.named_estimators_.items():
...
fig.suptitle('Evaluación del Voting Classifier - Breast Cancer', fontsize=14, fontweight='bold')
...
# Subplot 4: Comparación de clasificadores individuales
classifier_names = list(individual_scores.keys())
classifier_accuracies = list(individual_scores.values())
classifier_accuracies.append(test_accuracy)  # Añadir el voting classifier
classifier_names.append('Voting')

colors = plt.cm.Set3(np.linspace(0, 1, len(classifier_names)))
bars = axes[1, 1].bar(classifier_names, classifier_accuracies, color=colors, alpha=0.8)
...
for bar, acc in zip(bars, classifier_accuracies):
    height = bar.get_height()
    axes[1, 1].text(bar.get_x() + bar.get_width()/2., height + 0.005,
                    f'{acc:.3f}', ha='center', va='bottom', fontweight='bold', fontsize=8)

# Destacar el mejor clasificador
best_idx = np.argmax(classifier_accuracies)
...
# Gráfica adicional: Curvas de aprendizaje
plt.figure(figsize=(10, 6))
train_sizes, train_scores, val_scores = learning_curve(
    best_voting, X_train_scaled, y_train, cv=cv_strategy, 
...
plt.title('Curvas de Aprendizaje - Voting Classifier', fontsize=14, fontweight='bold')
...
print(f"Algoritmo: Voting Classifier")
print(f"Tipo de votación: {best_voting.voting}")
print(f"Clasificadores utilizados: {len(best_voting.estimators_)}")
...
print(f"Mejor clasificador individual: {max(individual_scores, key=individual_scores.get)} ({max(individual_scores.values()):.4f})")
print(f"Voting Classifier: {test_accuracy:.4f}")
improvement = test_accuracy - max(individual_scores.values())
